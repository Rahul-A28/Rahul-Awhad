...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.auth.verifyUser().subscribe(data=>{
      ...
      this.http.post("http://localhost:3000/user",{"email":otherEmail}).subscribe((data)=>{
        ...
        this.dateValue = this.otherProfile.dob.split("-");
        this.dateValue = this.dateValue[2] + "-" + this.dateValue[1] + "-" + this.dateValue[0];
    },(err)=>console.log(err));
  ...
  saveProfile(saveData:any){
    ...
    this.http.post('http://localhost:3000/saveProfile',saveData).subscribe((data)=>{alert("Updated Successfully");this.route.navigate(["/dashboard/viewProfile",this.otherProfile.email])},err=>alert("Something Went Wrong"));
  }
